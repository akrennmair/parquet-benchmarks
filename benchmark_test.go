...
func BenchmarkIssue84(b *testing.B) {
	...
	b.Run("parquet_go_floor_reflection", func(b *testing.B) {
		for n := 0; n < b.N; n++ {
			func() {
				...
				parquetFilename := "parquet_go_floor_reflection.parquet"
	...
	b.Run("parquet_go_floor_marshalling", func(b *testing.B) {
		for n := 0; n < b.N; n++ {
			func() {
				...
				parquetFilename := "parquet_go_floor_marshalling.parquet"
	...
	b.Run("parquet_go_lowlevel", func(b *testing.B) {
		for n := 0; n < b.N; n++ {
			func() {
				...
				parquetFilename := "parquet_go_lowlevel.parquet"
	...
	b.Run("xitongsys_parquet_go", func(b *testing.B) {
		for n := 0; n < b.N; n++ {
			func() {
				var err error
				w, err := os.Create("xitongsys_parquet_go.parquet")
				if err != nil {
	...
	b.Run("apache_arrow_parquet", func(b *testing.B) {
		for n := 0; n < b.N; n++ {
			func() {
				w, err := os.OpenFile("apache_arrow_parquet.parquet", os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0644)
				if err != nil {
